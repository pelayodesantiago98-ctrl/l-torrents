...
async function buscar(query, { idioma = "", calidad = "", pagina = 1 } = {}) {
  ...
  // Filtro de idioma estricto: solo lo que podemos confirmar que es ese idioma
  // (campo Language de 1337x, o pista clara en el nombre). Los de idioma
  // desconocido NO pasan: dejarlos pasar hacia que el filtro no filtrase nada,
  // porque PirateBay y TorrentProject no dan idioma.
  if (idioma) {
    items = items.filter((x) => x.idioma === idioma);
  }
  if (calidad) {
