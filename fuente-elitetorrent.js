...
for (const nombre of FUNCIONES) {
  ...
  module.exports[nombre] =
    typeof original === "function"
      ? async (...args) => {
          // El paquete peta con un TypeError opaco cuando una pagina no tiene
          // la forma que espera (p.ej. rutas que ahora redirigen a la home).
          // Se traduce a un error legible en vez de un 502 sin pista.
          try { return limpiar(await original.apply(et, args)); }
          catch (e) { throw new Error(`elitetorrent (${nombre}): ${e.message}`); }
        }
      : original;
}
